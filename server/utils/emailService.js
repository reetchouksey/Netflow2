...
const getTransporter = () => {
  if (!process.env.SMTP_HOST && !process.env.SMTP_USER && !process.env.SMTP_SERVICE) {
    return null
  }

  const port = Number(process.env.SMTP_PORT) || 587
  const isGmail = (process.env.SMTP_HOST || '').includes('gmail.com') || process.env.SMTP_SERVICE === 'gmail'
...
const sendMail = async ({ to, subject, text, html }) => {
  ...
  if (!transporter) {
    console.log(`\n================== [EMAIL SERVICE (DEV/LOCAL FALLBACK)] ==================`)
    console.log(`To: ${to}`)
  ...
  await transporter.sendMail({
    from: `"${process.env.SENDER_NAME || 'NetFlow'}" <${process.env.SENDER_EMAIL || process.env.SMTP_USER || 'no-reply@netflow.app'}>`,
    to,
    subject,
    text,
    ...(html ? { html } : {})
  })
  console.log(`[Email Service] Real email sent successfully to: ${to}`)
}
